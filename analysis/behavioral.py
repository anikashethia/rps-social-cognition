...
if __name__ == "__main__":
    ...
    print("\nMonotonic gradient tests:")
    for measure in ["win_rate", "choice_entropy", "win_stay", "lose_shift", "autocorr_lag1"]:
        if measure in group_df.columns:
            result = test_monotonic_gradient(group_df, measure=measure)
            print(f"  {measure:20s}: r={result['mean_r']:.3f}, t={result['t_stat']:.2f}, p={result['p_value']:.3f}")
